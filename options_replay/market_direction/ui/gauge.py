...
def build_gauge_html(signal) -> str:
    ...
    # El gauge muestra la DIRECCIÓN (sesgo del mercado según el score), que concuerda con la aguja y
    # la zona coloreada. La ACCIÓN de trade (puede ser NO TRADE por un filtro) se muestra en el panel
    # al lado — así una dirección alcista con la aguja en verde ya no contradice un «NO TRADE».
    if score >= 75.0:
        direction, color = "ALCISTA", "#16a34a"
    elif score <= 25.0:
        direction, color = "BAJISTA", "#dc2626"
    else:
        direction, color = "NEUTRAL", "#6b7280"
    ...
    return (
        ...
        f' aria-label="Medidor de dirección: {direction}, fuerza {score:.0f} de 100">'
        ...
        f'<text x="{cx}" y="102" text-anchor="middle" font-size="30" font-weight="800" fill="{color}">{direction}</text>'
